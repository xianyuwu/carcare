...
# ───────────────────────────────────────────────
# 续页 Prompt：PDF 第2页，只提取续表中的保养项目
# ───────────────────────────────────────────────
CONTINUATION_PROMPT = """你是一名专业的汽车保养结算单信息抽取专家。这是一份多页结算单的后续页面，页面上可能有保养项目表格的续表、金额汇总等信息。

请识别图片中所有内容，严格按照以下 JSON 格式输出，不要输出任何 JSON 以外的文字：

{
  "items": [
    {
      "name": "项目名称",
      "part_number": "配件号（没有填空字符串）",
      "operation": "操作类型（更换/检查/清洗等）",
      "quantity": 1,
      "unit_price": 0,
      "parts_fee": 0,
      "labor_fee": 0,
...
注意：
- 【极其重要】必须完整提取表格中的所有行，不得遗漏任何一个项目！即使项目很多也必须全部提取
- 跳过纯分组父行（如单独的"保养"二字、"维修"二字等无具体内容的行）
- 取每一行最具体的操作项目（如"更换机油滤清器"、"四轮定位"等）
- quantity 为空默认 1，金额为空填 0
- 含零部件号的行务必抽取 part_number
- 所有金额为纯数字（不带 ¥/元）
- _items_bbox 数组与 items 数组顺序一一对应，每个元素为归一化坐标 [x1, y1, x2, y2]（范围 0~1，表示在图片上的相对位置）
- 如果页面上有金额汇总信息（合计、折扣、实付等），提取到 cost 字段，不要编造
- 无法识别的字段填 null，禁止编造"""
